...
export default function QueryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = new URL(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/query/stream`);
  ...
  return (
    <div className="container mx-auto py-10 h-[calc(100vh-80px)] flex gap-6 max-w-7xl">
      {/* Main Chat Area */}
      <div className={`flex flex-col flex-1 transition-all duration-300 ${isSidePanelOpen ? 'max-w-[50%]' : 'max-w-4xl mx-auto'}`}>
                        ...
                        components={{
                          p: ({ children }) => {
                            if (typeof children === 'string' || Array.isArray(children)) {
                              const process = (child: React.ReactNode): React.ReactNode => {
                                ...
                                return parts.map((part, i) => {
                                  ...
                                  if (match) {
                                    ...
                                    const source = msg.sources?.find(s => s.sourceNumber === sourceNum);
                                    if (source) {
                                      return (
                                        <button
                                          key={i}
                              ...
                              return <p>{Array.isArray(children) ? children.map(process) : process(children)}</p>;
                            }
                            return <p>{children}</p>;
                          }
                        }}
                    ...
                    <div className="w-full mt-2 pl-12 max-w-[90%] flex flex-wrap gap-2">
                      {msg.sources.map((source, sIdx) => (
                        <Button
                          ...
                          onClick={() => handleSourceClick(source)}
                        >
                          <span className="font-bold border rounded px-1 min-w-[1.2rem] text-center">{source.sourceNumber || sIdx + 1}</span>
                          Source
                        </Button>
                      ))}
                    </div>
        ...
        <Card className="w-[50%] flex flex-col border-l-2 shadow-2xl animate-in slide-in-from-right duration-300">
          <div className="p-4 border-b flex justify-between items-center bg-muted/30">
            <h3 className="font-bold truncate max-w-[80%] text-primary">
              Source {selectedSource?.sourceNumber}: {selectedSource?.documentId ? selectedSource.documentId.split('/').pop() : 'Direct Reference'}
            </h3>
            <Button variant="ghost" size="sm" onClick={() => setIsSidePanelOpen(false)} className="rounded-full hover:bg-muted text-xl font-light">×</Button>
          </div>
          
          <ScrollArea className="flex-1">
            <div className="p-8 space-y-6">
              <div className="space-y-2">
                <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground/60">Source Excerpt</h4>
                <div className="relative group">
              ...
              {selectedSource?.metadata?.page && (
                <div className="flex items-center gap-4 py-4 border-t border-dashed">
                  <div>
                    <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground/60 block mb-1">Document Information</span>
          ...
          <div className="p-6 bg-muted/10 border-t mt-auto">
            <p className="text-[10px] text-muted-foreground/50 text-center uppercase tracking-[0.2em] font-medium">
              Reference Information • EcoReady Retrieval Engine
            </p>
          </div>
        </Card>
      )}
